function calculations(X, y, withLambdaIteration = false) 
  [m, n] = size(X);
  ...
  buy = find(trainingResults==1);
  strongBuy = find(trainingResults==2);
  sell = find(trainingResults==3);
  keep = find(trainingResults==4);

  % display obtained data
  figure; hold on;

  % 1 and 3 are choosen by hand, it is easy to see irises on plot
  plot(trainingSet(keep, 5), trainingSet(keep, 1), 'ko', 'LineWidth', 2, 'MarkerFaceColor', 'g', 'MarkerSize', 4);
  plot(trainingSet(buy, 5), trainingSet(buy, 1), 'k+', 'LineWidth', 2, 'MarkerSize', 4);
  plot(trainingSet(strongBuy, 5), trainingSet(strongBuy, 1), 'k*', 'MarkerFaceColor', 'y', 'MarkerSize', 4);
  plot(trainingSet(sell, 5), trainingSet(sell, 1), 'ko', 'MarkerFaceColor', 'r', 'MarkerSize', 4);
